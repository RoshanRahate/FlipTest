...
const TransactionsScreen = (props) => {
    ...
    const filterResults = (text) => {
        ...
        let updatedData = data.filter(item => {
            return item.beneficiary_name.toLowerCase().includes(searchedtext) || item.amount.toString().includes(searchedtext) || 
            item.sender_bank.toLowerCase().includes(searchedtext) || item.beneficiary_bank.toLowerCase().includes(searchedtext)
        })
    ...
    const sortDataWith = (type, array) => {
        ...
        switch (type) {
            case "Name A-Z":
                sortedArr = array.sort((a, b) => (a.beneficiary_name.toLowerCase() > b.beneficiary_name.toLowerCase() ? 1 : -1))
                break;
            case "Name Z-A":
                sortedArr = array.sort((a, b) => (a.beneficiary_name.toLowerCase() < b.beneficiary_name.toLowerCase() ? 1 : -1))
                break;
            case "Date Newest":
                sortedArr = array.sort((a, b) => getDate(a.created_at) < getDate(b.created_at) ? 1 : -1)
                break;
            case "Date Oldest":
                sortedArr = array.sort((a, b) => getDate(a.created_at) > getDate(b.created_at) ? 1 : -1)
                break;
